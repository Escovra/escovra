...
/// @title AgenticCommerce — ERC-8183 on Base (Escovra)
/// @notice First ERC-8183 marketplace with Web UI. For humans and AI agents.
    ...
    UUPSUpgradeable
{
    ...
    event JobCreated(uint256 indexed jobId, address indexed client, address indexed provider, address evaluator, uint256 expiredAt, address hook, string description);
    ...
    function getJobsBatch(uint256 from, uint256 to) external view returns (Job[] memory) {
        if (to > jobCounter) to = jobCounter;
        Job[] memory result = new Job[](to - from + 1);
        for (uint256 i = from; i <= to; i++) {
            result[i - from] = jobs[i];
        }
        return result;
    }
}
